<template>
  ...
  import { createMaterialCategory, updateMaterialCategory } from '@/api/material/material-category'
  import type { CreateUpdateMaterialCategoryQueryDto } from '@/api/material/material-category.d'
  ...
  function save() {
    if (props.handleType === 'add') {
      formRef.value.submit().then((res: CreateUpdateMaterialCategoryQueryDto) => {
        createMaterialCategory(res).then((response) => {
      ...
      formRef.value.submit().then((res: CreateUpdateMaterialCategoryQueryDto) => {
        updateMaterialCategory(props.modelValue.id, res).then((response) => {
            console.log(response)
            close(true)
        })
      })
