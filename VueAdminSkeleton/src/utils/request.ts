...
export default function createAxios<R = any, Q = any>(opt?: RequestOption): any {
    ...
    if (e instanceof AxiosError) {
      ...
      if (responseData && typeof responseData === 'object') {
        if (responseData.message) {
          param.message = responseData.message
        } else if (responseData.error) {
          param.message = responseData.error
        } else {
          // 如果没有服务器错误信息，显示HTTP状态信息
          param.message = e.response?.status ? 
            `请求失败 (${e.response.status}): ${e.response.statusText || e.message}` : 
            e.message
        }
      } else {
  ...
  service.interceptors.response.use(
    async (res) => {
      const data = res.data
      resetLoading()
      // if (res.status && res.status !== 200) return showErrorMsg(data)
