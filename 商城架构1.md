...
- 品牌选择页面（一级 tab 页面），卡片形式展示拉取的品牌列表

  - 布局：左图片 logo；右边是名称、描述、官网三行，其中名称加粗展示
  - 交互：点击品牌后，跳转下面的「商城子页面」
- 商城子页面 （二级子页面）

  - 左侧是商品分类展示，使用 https://nutui.jd.com/taro/vue/4x/#/zh-CN/component/category 实现
  - 右侧是商品 SPU 列表，使用 https://nutui.jd.com/taro/vue/4x/#/zh-CN/component/card 实现 （这里先不展示价格，图片只用 main_material，需要展示标题、副标题、描述等信息）
- 商品详情页 （三级子页面）

  - 上方展示 SPU 轮播图，使用 https://nutui.jd.com/taro/vue/4x/#/zh-CN/component/swiper 实现，包括 main_material 和 sub_materials
  - 中间展示 SPU 的 detail_content
  - 底部有两个按钮，一个是加入购物车，一个是立即购买，点击后弹出 SKU 选择弹窗
- SKU 选择（弹窗）

  - 使用 https://nutui.jd.com/taro/vue/4x/#/zh-CN/component/sku 实现，图片暂时使用 main_material 的图片
