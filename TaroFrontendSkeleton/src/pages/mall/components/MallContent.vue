<template>
  <view class="sub-page">
    <nut-navbar :title="title" :left-show="enableBack" @on-click-back="goBack" safe-area-inset-top>
      <template #right>
...
const fetchCategories = async () => {
  ...
  if (props.brandId) {
    ...
    params.filters = { brand_id: props.brandId }
  } else {
    // 否则获取顶级分类
...
const fetchBrand = async () => {
  ...
  const { code, data } = await mallService.getBrand(props.brandId)
  if (code === 0 && data) {
...
const fetchSpus = async (reset = false) => {
  ...
  if (props.brandId) filters.brand_id = props.brandId
